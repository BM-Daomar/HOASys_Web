...
class Admin extends General
{
    ...
    public function get_committees()
    {
        ...
        $committees = $this->qry_committee($qry);
        echo json_encode($committees);
    }
    public function add_alumni(){
        $fullname = $this->input->post("fullname");
        $batch = $this->input->post("alumni_batch");
        $cellphone = trim($this->input->post("cellphone")) ?  trim($this->input->post("cellphone")) : null;
        $check['fullname'] = $fullname;
        if($cellphone != null){
            $check['cellphone'] = $cellphone;
        }
        $participant_exist = $this->qry_check_new_participant($check);
        if(count($participant_exist) > 0){
            echo json_encode(array('status' => 'error', 'message' => 'Participant Already Exist', 'details' => $participant_exist[0]));
        }else{
            $data["fullname"] = trim($fullname);
            $data["batch"] = $batch;
            $data["cellphone"] = $cellphone;
            $data["participantType"] = "Alumni";
            $insert_stat = $this->general_model->insert_vals($data, "tbl_participant");
            if($insert_stat){
                echo json_encode(array('status' => 'success', 'message' => 'Alumni participant is successfully added'));
            }else{
                echo json_encode(array('status' => 'error', 'message' => 'Error while adding alumni record'));
            }
        }
    }
    public function update_alumni(){
        ...
        $fullname = $this->input->post("fullname");
        $batch = $this->input->post("alumni_batch")?$this->input->post("alumni_batch"): null;
        $cellphone = trim($this->input->post("cellphone")) ?  trim($this->input->post("cellphone")) : null;
        $check['fullname'] = $fullname;
        $check['participant_ID'] = $participant_id;
        if(json_decode($this->input->post("convert"))){
            $data["participantType"] = "Alumni";
        }
        if($cellphone != null){
            $check['cellphone'] = $cellphone;
        ...
        if(count($participant_exist) > 0){
            ...
            $update_stat = $this->general_model->update_vals($data, $where, "tbl_participant");
            if($update_stat){
                $registration_details = $this->check_participant_registration($participant_id);
                echo json_encode(array('status' => 'success', 'message' => 'Alumni participant is successfully updated', 'details'=>$registration_details[0]));
            }else{
                echo json_encode(array('status' => 'error', 'message' => 'Error while updating alumni record'));
            }
        }
    }
