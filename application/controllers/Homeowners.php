...
class Homeowners extends General
{
    ...
    public function get_prizes()
    {
        ...
        $query['query'] = "SELECT prize_ID,name,winners,sequence,dateTimeSpin,registered,major,isActive FROM tbl_prize WHERE major IS NULL " . $where_name;
    ...
    public function get_prizes_major()
    {
        $datatable = $this->input->post('datatable');
        $query['search']['append'] = "";
        $query['search']['total'] = "";
        ...
        $query['query'] = "SELECT prize_ID,name,winners,sequence,dateTimeSpin,registered,major,isActive FROM tbl_prize WHERE major IS NOT NULL " . $where_name;
        if ($datatable['query']['searchField'] != '') {
            $keyword = $datatable['query']['searchField'];
            $where = "name LIKE '%" . $keyword . "%'";
            $query['search']['append'] = " AND ($where)";
    ...
    {
        $prize['name'] = $this->input->post('prize');
        $prize['winners'] = $this->input->post('winners');
        $dateTime = $this->get_current_date_time();
        $prize['dateCreated'] = $dateTime['dateTime'];
        $major = $this->input->post('major');
        $where_major = "";
        // if this sequence already existed
        if ($major == 1) {
            $prize['major'] = $major;
        ...
        $succeeding_prizes = $this->general_model->custom_query('SELECT sequence, prize_ID FROM `tbl_prize` WHERE sequence >= ' . $sequence . '' . $where_major);
        ...
        // if ($seq_res != null || $seq_res != "") {
        //     // Exchange
        //     $max = $this->general_model->custom_query('SELECT MAX(sequence) as seq FROM tbl_prize WHERE major = ' . $major);
        //     $new_max = $max[0]->seq + 1;
        ...
        // }
        $prize['sequence']  = $this->input->post('sequence');
        $registered = $this->input->post('registered');
        $active = $this->input->post('active');

        if ($registered == 1) {
            $prize['registered'] = $registered;
        }
        ...
        if ($active == 1) {
            ...
            $prize['isActive'] = $active;
        }
        $this->general_model->insert_vals($prize, "tbl_prize");
    }
    public function save_homeowner()
    {
        ...
        $this->email_sending($newinserted_id,$em);
    }
    // email sending back end
    public function email_sending($newinserted_id, $em)
    {
        ...
        $message = "Hi ! In order for you to logged in to on your GGN1 account, Kindly click the verification link: " .anchor($ser . '/homeowners/verify_account/'.$newinserted_id, '   VERIFY MY ACCOUNT');
    ...
    public function verify_account($id)
    {
        ...
        $this->general_model->update_vals($data, $where, 'tbl_homeowners');
